...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.ROI_LeftCorner_y = 0
        self.ROI_RightCorner_x = 0
        self.ROI_RightCorner_y =  0
    ...
    # def enter_taille_pixel(self):
    #     self.taillePixel = self.taille_pixel.text()
    #     if not self.taillePixel or self.taillePixel.isalpha() or self.taillePixel == '0' :
    #         self.notifications.setText("")
    #         self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    #     else:
    #         self.taillePixel = float(self.taillePixel)
    #         self.notifications.setText(" ")

    # def enter_freq_acquisition(self):
    #     self.frame_rate = self.freq_acquisition.text()
    #     if not self.frame_rate or self.frame_rate.isalpha():
    #         self.notifications.setText("")
    #         self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    #     else:
    #         self.notifications.setText(" ")

    # def enter_nouvelle_freq(self):
    #     self.new_frame_rate = self.nouvelle_freq.text()
    #     if not self.new_frame_rate or self.new_frame_rate.isalpha():
    #         self.notifications.setText("")
    #         self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    #     else:
    #         self.notifications.setText(" ")

    # def enter_diameter(self):
    #     self.diametre = self.diameter.text()
    #     if not self.diametre or self.diametre.isalpha():
    #         self.notifications.setText("")
    #         self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    #     else:
    #         self.notifications.setText(" ")

    # def enter_timestamps_start(self):
    #     self.timestamp = self.timestamps_start.text()
    #     if not self.timestamp or self.timestamp.isalpha():
    #         self.notifications.setText("")
    #         self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    #     else:
    #         self.notifications.setText(" ")

    def update_view(self,image,placement):
    ...
    def update_view_2(self,image,placement):
        rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image_height, image_width, ch = rgb_image.shape
        bytes_per_line = ch * image_width
        convert_to_Qt_format = QImage(rgb_image.data, image_width, image_height, bytes_per_line, QImage.Format_RGB888)
    ...
    def on_reset_button_clicked(self):
        try:
            ...
            self.notifications.setText("Sélectionnez une image d'abord")
            self.notifications.setFont(QFont("Times", 10, QFont.Bold))
    ...
    def on_roi_selection_complete(self,x0,y0,x1,y1,x0_test,y0_test):
            ...
            self.ROI_width = abs(self.ROI_RightCorner_x - self.ROI_LeftCorner_x)
            self.ROI_height = abs(self.ROI_RightCorner_y - self.ROI_LeftCorner_x)
            ...
            self.ROI_image = self.original_image[self.ROI_LeftCorner_y:self.ROI_RightCorner_y,self.ROI_LeftCorner_x:self.ROI_RightCorner_x]
            self.update_view(self.ROI_image,self.image_holder)
    ...
    def on_template_selection_complete(self,x0,y0,x1,y1,x0_test,y0_test):
            ...
            self.template_LeftCorner_y = int((y0*self.ROI_image.shape[0])/self.image_holder.height())
            self.template_RightCorner_x = int((x1*self.ROI_image.shape[1])/self.image_holder.width())
            self.template_RightCorner_y =  int((y1*self.ROI_image.shape[0])/self.image_holder.height())
    ...
    def position_per_time_calculation(self):
        ...
        for i in range(0,len(images_filenames)-1):
            ...
            if images_equals == False:
                counter += 1
                if (self.ROI_LeftCorner_x + self.ROI_width + np.sum(displacements) < self.original_image_width):

                    self.ROI_LeftCorner_x = int(self.ROI_LeftCorner_x + np.sum(displacements))
                    self.ROI_RightCorner_x = int(self.ROI_RightCorner_x + np.sum(displacements))

                    img1=img1[self.ROI_LeftCorner_y:self.ROI_RightCorner_y,self.ROI_LeftCorner_x:self.ROI_RightCorner_x]
                    img2=img2[self.ROI_LeftCorner_y:self.ROI_RightCorner_y,self.ROI_LeftCorner_x:self.ROI_RightCorner_x]
                    ...
                    self.ROI_LeftCorner_x =self.original_image_width - self.ROI_width
                ...
                ROI_xLeft_coordinates.append(self.ROI_LeftCorner_x)
            ...
            corr_array = correlation_ROI_template[0,:]
            x = np.arange(len(corr_array))
            print("min loc",min_loc[0])
            fit_params=np.polyfit(x[min_loc[0]-10:min_loc[0]+10],corr_array[min_loc[0]-10:min_loc[0]+10],6) #polyfit: x, y, deg:50 
            
            #create a polymnome using fit_params with x_fit : x abs of 0.0001 resolution and y_fit from x_fit and fit_params
            x_fit = np.arange(min_loc[0]-10,min_loc[0]+10-1,0.0001) 
    ...
    def flow_velocity(self):
        ...
        # print("A=",self.A,"  B=",self.B) # A: vitesse self.A*pixel_size/self.n_resize
        # for time in self.time_instants:
        #     self.predicted_positions.append(self.A*int(time)+ self.B)
        # print(len(self.predicted_positions))
        # #velocity(µm/s) = A*pixel_size
        # return self.A*self.taillePixel, residuals*(self.taillePixel)**2
    ...
    def enregistrer_recap(self):
            ...
            print("Taille de pixel (µm)=", self.taillePixel)
            print("Diametre interne du capillaire (µm)=", self.diametre)
            ...
            print("Flow velocity (µm/s)=", self.velocity)
            print("flow rate (µl/min)=",self.flow_rate)
            ...
            output = {"positions":self.positions, "time_instants": self.time_instants}
            output_data = pd.DataFrame(data=output)
            with open("postion_time_data.txt", '+w') as f:
                f.write(" ---------------------- fichier contenant les positions de la caméra et les instants correspondants ---------------------- ")
            ...
            # self.plot_data.canvas.ax3.hist(self.flow_rate_dynm, bins='auto',color='dodgerblue',edgecolor='black')
            # self.plot_data.canvas.draw()
