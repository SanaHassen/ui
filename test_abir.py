...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def on_roi_selection_complete(self,x0,y0,x1,y1,x0_test,y0_test):
        try:
            ...
            self.ROI_height = abs(self.ROI_RightCorner_y - self.ROI_LeftCorner_y)
    ...
    def position_per_time_calculation(self):
        ...
        for i in range(0,len(images_filenames)-1):
            ...
            print("sum disp",np.sum(displacements) )
            ...
            if images_equals == False:
                counter += 1
                if (self.ROI_LeftCorner_x + self.ROI_width + int(np.sum(displacements)) < self.original_image_width):
                    print("case 1",self.ROI_LeftCorner_x)

                    self.ROI_LeftCorner_x = int(self.ROI_LeftCorner_x + int(np.sum(displacements)))
                    self.ROI_RightCorner_x = int(self.ROI_RightCorner_x + int(np.sum(displacements)))
                    ...
                    print("case 2", self.ROI_LeftCorner_x)
                ...
                # ROI_xLeft_coordinates.append(self.ROI_LeftCorner_x)
                    ...
                    print("template",new_template_LeftCorner_x)
            ...
            if i == 3:
                self.corr_array_1 = np.asarray(correlation_ROI_template[0,:])
                self.x_1 = np.arange(len(self.corr_array_1))
                min_cor=min(self.corr_array_1)
                min_loc=self.x_1[np.where(self.corr_array_1==min_cor)[0]][0] 
                print("min loc",min_loc)
            #fit_params=np.polyfit(self.x_1[min_loc-30:min_loc+30],self.corr_array_1[min_loc-30:min_loc+30],50) #polyfit: x, y, deg:50 
            else:
                self.corr_array = np.asarray(correlation_ROI_template[0,:])
                self.x = np.arange(len(self.corr_array))
                min_cor=min(self.corr_array)
                min_loc=self.x[np.where(self.corr_array==min_cor)[0]][0] 
                print("min loc",min_loc)

          
    
    def flow_velocity(self):
        self.position_per_time_calculation()
    ...
    def enregistrer_recap(self):
            ...
            self.plot_data.canvas.ax1.plot(self.x_1, self.corr_array_1, '-r', c='dodgerblue', label='Linear fit')
